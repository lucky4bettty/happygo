		...
		<table class="tbType" cellspacing="0" cellpadding="0" border="0"
			...
			<tbody>
				...
				<tr>
					...
					<td><form:input path="text_color" maxlength="50" /> <form:errors
							path="text_color" cssClass="error" /></td>
				</tr>
				<tr>
					<th scope="row"><span class="red">※</span>游標效果顏色</th>
					<td><form:input path="cursor_color" maxlength="50" /> <form:errors
							path="cursor_color" cssClass="error" /></td>
				</tr>
				<tr>
					<th scope="row">顯示ICON</th>
					<td><form:radiobuttons path="icon_show" items="${iconOption}" />
						<form:errors path="icon_show" cssClass="error" /></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><form:radiobuttons path="type" items="${typeOption}" /> <form:errors
							path="type" cssClass="error" /></td>
				</tr>

				<tr>
					<th scope="row"><span class="red">※</span>主選單連結</th>
					<td><form:input path="main_url" maxlength="50" /> <form:errors
							path="main_url" cssClass="error" /></td>
				</tr>
				<tr>
					<th scope="row"><span class="red">※</span>廣告名稱(最多20字)</th>
					<td><form:input path="ad_name" maxlength="50" /> <form:errors
							path="ad_name" cssClass="error" /></td>
				</tr>
				<tr class="bg">
					<th scope="row"><span class="red">※</span>內容</th>
					<td><form:textarea path="ad_content" rows="10" cols="20"
							style="margin: 0px; width: 400px; height: 300px;" /> <form:errors
							path="ad_content" cssClass="error" /></td>
				</tr>
				<tr>
					<th scope="row"><span class="red">※</span>圖片</th>
					<td><form:hidden path="image" /> <input type="file"
						name="img_file" accept=".jpg,.gif,.png" /> <c:if
							test="${not empty command.image}">
							<img
								src="<c:out value="${imagePath}"/><c:out value="${command.image}"/>"
								width="100" />
						</c:if> <form:errors path="image" cssClass="error" />
						<div>1.圖檔格式只支援GIF,JPG,PNG三種格式,其他格式,系統將略過不處理!</div>
						<div>2.新上傳圖檔,會自動將舊圖檔直接覆蓋!</div>
						<div>
							3.圖檔建議尺寸為<span class="red">寬1960 *高 420</span>
						</div></td>
				</tr>
