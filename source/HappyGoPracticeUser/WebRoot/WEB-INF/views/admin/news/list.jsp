          ...
          <c:forEach items="${page.pageItems}" var="data" varStatus="st">
			  <tr  <c:if test="${st.count % 2 ==0}">class="bg"</c:if>>
				...
				<td align="center" width="5%">
	              	<select name="sort" id="sort" onchange="updateSort('<c:out value="${data.id}"/>', this.value, ${data.sort});">
				...
				<td align="center" width="5%" style="display:none;">
					<form:input path="sortTemp" maxlength="100" style="width:400px;" value="${data.sort}"/>
				</td>
				<td align="center" width="15%"><c:out value="${data.name}"/></td>
				<td align="center" width="15%"><img src="<c:out value="${imagePath}"/><c:out value="${data.image}"/>" width="100"/></td>
