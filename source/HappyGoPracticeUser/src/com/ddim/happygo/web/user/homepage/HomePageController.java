package com.ddim.happygo.web.user.homepage;
...
import com.ddim.happygo.web.admin.nav.HeaderFormValidator;
...
public class HomePageController {
	...
	protected String getRightID() {
		return Constants.HG_MENU_MAIN_LIST;
	}
	...
	private void setCommonData(HttpServletRequest request) throws Exception {
		...
		request.setAttribute("subMenuList", menuSubService.getList(Constants.STATUS_ENABLE));
		request.setAttribute("headerList", navHeaderService.getList(Constants.STATUS_ENABLE));
		request.setAttribute("footerList", navFooterService.getList(Constants.STATUS_ENABLE));
	}
	...
	public String list(HttpServletRequest request, @ModelAttribute("command") MenuMainForm form, PagingParameter paging,
...
//			Page<MenuMain> page = menuMainService.findPage(form.getName(), form.getStatus(), paging);
...
//			model.addAttribute("page", page);
	...
	public String edit(HttpServletRequest request, @ModelAttribute("command") MenuMainForm form, ModelMap model)
			throws Exception {
		try {
			String returnPath = managerUtil.haveRight(request, getRightID());
			if (returnPath != null) {
			...
			if (StringUtils.isNotBlank(form.getId())) {
				MenuMain data = menuMainService.get(form.getId());
				if (data == null) {
	...
	public String save(HttpServletRequest request, @ModelAttribute("command") MenuMainForm form, BindingResult result,
			ModelMap model) throws Exception {
		try {
			String returnPath = managerUtil.haveRight(request, getRightID());
			if (returnPath != null) {
			...
			BeanUtils.copyProperties(form, menuMain);

			menuMainService.save(menuMain, managerUtil.getManagerAccount(request));
			LogUtil.setSaveLog(managerUtil.getManagerAccount(request),
					"save manager success: " + menuMain.getId());
	...
	public String update(HttpServletRequest request, @ModelAttribute("command") MenuMainForm form, ModelMap model)
			throws Exception {
		try {
			String returnPath = managerUtil.haveRight(request, getRightID());
			if (returnPath != null) {
			...
			menuMainService.updateStatus(form.getIds(), form.getStatus(), managerUtil.getManagerAccount(request));
			LogUtil.setSaveLog(managerUtil.getManagerAccount(request),
	...
	public String delete(HttpServletRequest request, @ModelAttribute("command") MenuMainForm form, ModelMap model)
			throws Exception {
		try {
			String returnPath = managerUtil.haveRight(request, getRightID());
			if (returnPath != null) {
			...
			menuMainService.delete(form.getIds(), managerUtil.getManagerAccount(request));
			LogUtil.setSaveLog(managerUtil.getManagerAccount(request),
    ...
    public @ResponseBody String getSunMenu(HttpServletRequest request, Object data) throws Exception {
    	System.out.println(data);
    	menuSubService.getList(Constants.STATUS_ENABLE);
